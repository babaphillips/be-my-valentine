...
const gifs = {
  ...
  happy: new URL(
    "../assets/c616fca52915df67140d1ce0e002ce5a.gif",
    import.meta.url,
  ).href,
};

const currentGif = computed(() => (accepted.value ? gifs.happy : gifs.begging));
...
watch(noClickCount, (count) => {
  alert.value = true;
  warningMessage.value = messages[Math.min(count - 1, messages.length - 1)];
